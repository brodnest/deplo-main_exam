...
//        Serial.print(g_num_of_nodes); Serial.println(" received / expected frames.");
  ...
  //Serial.print(count);
  count_good = count_good_frames(g_can_buffer);
  //Serial.print("good / responding nodes: "); #originally naka uncomment
  //Serial.print(count_good);Serial.print(" / ");
  //Serial.print(count); hanggang dito
...
int count_good_frames(CAN_FRAME can_buffer[]){
  int i = 0,count = 0;
  ...
  for (i=0; i< CAN_ARRAY_BUFFER_SIZE; i++){
    if (can_buffer[i].id != 0){
      ...
      id = can_buffer[i].id;
      d1 = can_buffer[i].data.byte[0];
      d2 = can_buffer[i].data.byte[1];
      d3 = can_buffer[i].data.byte[2];
      d4 = can_buffer[i].data.byte[3];
      d5 = can_buffer[i].data.byte[4];
      d6 = can_buffer[i].data.byte[5];
      d7 = can_buffer[i].data.byte[6];
      d8 = can_buffer[i].data.byte[7]; 
   
      x = compute_axis(d2,d3);
      y = compute_axis(d4,d5);
      z = compute_axis(d6,d7);

      mag =sqrt(float(sq(x)+sq(y)+sq(z)));
      //Serial.println(mag);
        if ((g_sensor_version<=4 && mag>=1004.0) && (mag<=1044.0)){
        count++;
        }
        else if ((g_sensor_version>=5 && mag>=12958.0) && (mag<=13358.0)){
        count++;
        }
        else{
          Serial.print("Error:");
...
void interpret_frame(CAN_FRAME incoming){
  ...
  if ((d1 == 11)|(d1 == 12)|(d1==32)|(d1==33)|(d1==41)|(d1==42)|(d1==51)|(d1==52)){ 
    ...
    Serial.print("\tV: ");
    Serial.print(2.00+(float(d8)/100));
    Serial.print("\tmag: ");
    Serial.print(sqrt(float(sq(x)+sq(y)+sq(z))));
    Serial.println();
